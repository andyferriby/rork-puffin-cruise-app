...
export default function TicketsScreen() {
  ...
  return <View style={styles.root}><ScrollView contentContainerStyle={{ paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
    ...
    {data.length === 0 ? <View style={styles.empty}><Ticket size={42} color={theme.textMuted}/><Text style={styles.emptyTitle}>No tickets loaded</Text><Text style={styles.emptyText}>Enter your booking email to show your boarding passes.</Text></View> : <View style={styles.list}>{data.map((b) => <View key={b.id} style={[styles.ticket, b.status === "boarded" && styles.ticketBoarded]}><View style={styles.ticketTop}><View style={{ flex: 1 }}><Text style={styles.cruise}>{b.cruise_name}</Text><Text style={styles.meta}>{new Date(b.cruise_date).toLocaleDateString("en-GB", { weekday: "short", day: "numeric", month: "short" })} · {b.cruise_time}</Text><Text style={styles.meta}>{b.adults} adult{b.adults === 1 ? "" : "s"} · {b.children} child{b.children === 1 ? "" : "ren"}</Text></View>{b.status === "boarded" ? <View style={styles.usedStamp}><CheckCircle size={16} color={theme.white} /><Text style={styles.usedStampText}>BOARDED</Text></View> : <View style={styles.qr}><Image source={{ uri: qrCodeUrl(b.id) }} style={{ width: 68, height: 68 }} contentFit="contain" /></View>}</View><View style={styles.dashed}/><View style={styles.ticketBottom}><Text style={[styles.status, b.status === "boarded" && styles.statusBoarded]}>{b.status === "boarded" ? "✓ BOARDED" : b.status.toUpperCase()}</Text>{b.status === "boarded" ? <View style={styles.boardedBadge}><Anchor size={12} color={theme.sea} /><Text style={styles.boardedBadgeText}>Already boarded</Text></View> : <Pressable onPress={() => openWalletPass(b.id)} style={styles.walletButton}><Wallet size={14} color={theme.white}/><Text style={styles.walletButtonText}>Add to Wallet</Text></Pressable>}</View></View>)}</View>}
  </ScrollView></View>;
}
const styles = StyleSheet.create({ root:{flex:1,backgroundColor:theme.bg}, header:{paddingHorizontal:20,paddingBottom:14}, title:{fontSize:34,fontWeight:"900",color:theme.text}, subtitle:{marginTop:4,color:theme.textMuted,fontSize:14}, searchCard:{margin:16,padding:16,borderRadius:20,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border,gap:12}, field:{height:48,borderRadius:14,backgroundColor:theme.bg,flexDirection:"row",alignItems:"center",gap:10,paddingHorizontal:12}, input:{flex:1,color:theme.text,fontSize:15}, button:{height:48,borderRadius:14,backgroundColor:theme.sea,alignItems:"center",justifyContent:"center"}, buttonText:{color:theme.white,fontWeight:"900",fontSize:15}, empty:{alignItems:"center",padding:36,margin:16,borderRadius:22,backgroundColor:theme.white,borderWidth:1,borderColor:theme.border}, emptyTitle:{marginTop:14,fontSize:18,fontWeight:"900",color:theme.text}, emptyText:{marginTop:6,textAlign:"center",color:theme.textMuted,lineHeight:20}, list:{padding:16,gap:14}, ticket:{backgroundColor:theme.white,borderRadius:22,padding:16,borderWidth:1,borderColor:theme.border}, ticketBoarded:{opacity:0.82,backgroundColor:"#F0F5FA"}, ticketTop:{flexDirection:"row",justifyContent:"space-between",gap:12}, cruise:{fontSize:18,fontWeight:"900",color:theme.text}, meta:{marginTop:4,color:theme.textMuted,fontSize:13}, qr:{width:78,height:78,borderRadius:16,backgroundColor:theme.foam,alignItems:"center",justifyContent:"center"}, usedStamp:{width:78,height:78,borderRadius:16,backgroundColor:theme.sea,alignItems:"center",justifyContent:"center",gap:3}, usedStampText:{color:theme.white,fontSize:10,fontWeight:"900",letterSpacing:0.5}, statusBoarded:{color:theme.sea}, dashed:{borderStyle:"dashed",borderTopWidth:1,borderColor:theme.border,marginVertical:14}, ticketBottom:{flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:8}, status:{color:theme.coral,fontSize:12,fontWeight:"900"}, walletButton:{minHeight:36,paddingHorizontal:12,borderRadius:999,backgroundColor:theme.sea,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:6}, walletButtonText:{fontSize:12,color:theme.white,fontWeight:"900"}, boardedBadge:{minHeight:36,paddingHorizontal:12,borderRadius:999,backgroundColor:theme.foam,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:5}, boardedBadgeText:{fontSize:11,color:theme.sea,fontWeight:"800"} });
